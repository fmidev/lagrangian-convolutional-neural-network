...
try:
    ...
    print("No pytorch installation found")
    pass
...
try:
    ...
    print("No models module found.")


class LCNNPrediction(PredictionBuilder):
    def __init__(self, config: AttrDict):
        super().__init__(config)
        model_path = config.model_path
        modelconf = config.modelconf
        self.model = LCNN.load_from_checkpoint(model_path, config=modelconf)
        self.model.eval()
    ...
    def preprocessing(self, data: np.ndarray, metadata: dict, params: AttrDict = None):
        """
    ...
    def nowcast(self, data: np.ndarray, params: AttrDict = None):
        "Pytorch LCNN Nowcast"
        ...
        # if params is None:
        #     params = AttrDict({"n_leadtimes": 24, "im_size": [512, 512]})
    ...
    def postprocessing(self, nowcast, params):
        ...
        if params is None:
            params = AttrDict({"zerovalue": 0, "nan_to_zero": True})
